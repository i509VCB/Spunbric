	...
	private static class FieldData {
		...
		void update(boolean increment) {
			...
			if (this.listenerCount < 0) {
				SpunbricImpl.getLogger().error(String.format("Decremented listener count to %s for field %s", this.listenerCount, this.field), new Exception("Dummy exception"));
			}
			...
			try {
				...
				SpunbricImpl.getLogger().error(String.format("Error setting field %s to %s", this.field, val), e);
			}
		}
	}
