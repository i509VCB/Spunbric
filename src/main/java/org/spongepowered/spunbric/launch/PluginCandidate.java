...
public final class PluginCandidate {
    ...
    public boolean collectDependencies(Map<String, String> loadedPlugins, Map<String, PluginCandidate> candidates) {
        ...
        for (PluginDependency dependency : this.metadata.collectRequiredDependencies()) {
            ...
            if (this.id.equals(id)) {
                FabricLaunch.getLogger().warn("Plugin '{}' from {} requires itself to be loaded. "
        ...
        if (loadAfter != null && !loadAfter.isEmpty()) {
            this.invalid = true;
            FabricLaunch.getLogger().error("Invalid dependency with load order AFTER on plugin '{}' from {}. "
    ...
    private void collectOptionalDependencies(@Nullable Iterable<PluginDependency> dependencies,
        ...
        for (PluginDependency dependency : dependencies) {
            final String id = dependency.getId();
            if (this.id.equals(id)) {
                FabricLaunch.getLogger().error("Plugin '{}' from {} cannot have a dependency on itself. This is redundant and should be "
    ...
    private boolean verifyVersionRange(String id, @Nullable String expectedRange, @Nullable String version) {
        ...
        if (version != null) {
            try {
                    ...
                    if (range.getRecommendedVersion() instanceof DefaultArtifactVersion) {
                        ...
                        if (majorExpected != null) {
                            ...
                            if (majorInstalled != null
                                ...
                                FabricLaunch.getLogger().warn("Plugin {} from {} was designed for {} {}. It may not work properly.",
                ...
                FabricLaunch.getLogger().error("Failed to parse version range {} for dependency {} of plugin {} from {}: {}",
                        version, id, this.id, this.source, e.getMessage());
                this.invalid = true;
            }
        } else {
            if (this.dependenciesWithUnknownVersion.add(id)) {
                FabricLaunch.getLogger().warn("Cannot check version of dependency {} for plugin {} from {}: Version of dependency unknown",
