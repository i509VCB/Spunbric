...
final class FabricPluginScanner {
    ...
    void scanClassPathKnot(ClassLoader loader, boolean scanJars) throws IOException {
        Set<URI> sources = new HashSet<>();
    ...
    void scanJar(Path path, boolean classpath) {
        ...
        try (JarInputStream jar = new JarInputStream(new BufferedInputStream(Files.newInputStream(path)))) {
            ...
            // TODO
            /* -- Sorry we don't have support for access transformers.
            if (manifest != null) {
        ...
        /* -- One again we do not support access transformers in this context.
        ...
        */
        ...
        if (!candidates.isEmpty()) {
            ...
            logger.error("No valid plugins found in {}. Is the file actually a plugin JAR? Please keep in mind Forge mods can be only loaded on "
                    + "SpongeForge servers, SpongeVanilla supports only Sponge plugins.", path); // TODO remove forge name
        }
    }
